...
fp_toString:					; LAB_296E
	ldy #1						; Set the character index
...
f2s_moreThan1:					; LAB_299A
	sta flt_numExp				; store off the exponent (0 or -6)

f2s_loop1:						; LAB_299C
	lda #<f_maxNonSci			; point to low byte of 999999.4375
	ldy #>f_maxNonSci			; (max value before scientific notation)
	jsr fp_compare				; LAB_27F8
	beq f2s_digits				; equal, just do digits (LAB_29C3)
	bpl f2s_div10				; do /10 if FP1 > (AY) (LAB_29B9)

f2s_loop2:						; LAB_29A7
	lda #<f_maxDec				; point to low byte of 99999.9375
	ldy #>f_maxDec				; (max value with at least one decimal place)
	jsr fp_compare				; LAB_27F8
	beq f2s_mul10				; = : allow decimal places
	bpl f2s_digits				; > : just do digits (LAB_29B2)
	
f2s_mul10:
	jsr fp_mult10				; do *10 since FP1 < (AY) (LAB_29B2)
...
f2s_digits:						; LAB_29C0 / LAB_29C3
	jsr fp_tofixed				; convert F1 to fixed (LAB_2831))
...
f2s_terminate:
	...
	iny
	sta (flt_vec),y				; store the terminator
	rts
